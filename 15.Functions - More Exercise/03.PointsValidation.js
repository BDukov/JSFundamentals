function pointsValidation(array) {

    let x1 = Number(array[0]);
    let y1 = Number(array[1]);
    let x2 = Number(array[2]);
    let y2 = Number(array[3]);
    let distanceBetweenTwoPoints = Math.sqrt(((x2 - x1) * (x2 - x1)) + ((y2 - y1) * (y2 - y1)));
    let orderOfComparison = Math.sqrt(((0 - x1) * (0 - x1)) + ((0 - y1) * (0 - y1)));
    let orderOfComparisonTwo = Math.sqrt(((x2 - 0) * (x2 - 0)) + ((y2 - 0) * (y2 - 0)));


    if (Number.isInteger(orderOfComparison)) {
        console.log(`{${x1}, ${y1}} to {0, 0} is valid`);
    } else {
        console.log(`{${x1}, ${y1}} to {0, 0} is invalid`);
    }
    if (Number.isInteger(orderOfComparisonTwo)) {
        console.log(`{${x2}, ${y2}} to {0, 0} is valid`);
    } else {
        console.log(`{${x2}, ${y2}} to {0, 0} is invalid`);
    }
    if (Number.isInteger(distanceBetweenTwoPoints)) {
        console.log(`{${x1}, ${y1}} to {${x2}, ${y2}} is valid`);
    } else {
        console.log(`{${x1}, ${y1}} to {${x2}, ${y2}} is invalid`);
    }
}
pointsValidation([3, 0, 0, 4]);